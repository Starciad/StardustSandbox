    ...
    internal sealed class SCloudHandler(ISGame gameInstance) : SGameObject(gameInstance), ISCloudHandler
    {
        ...
        public override void Update(GameTime gameTime)
        {
            ...
            for (int i = 0; i < this.activeClouds.Count; i++)
            {
                ...
                if (!this.SGameInstance.CameraManager.InsideCameraBounds(cloud.Position, new SSize2(cloud.Texture.Width, cloud.Texture.Height), false, cloud.Texture.Width + (SWorldConstants.GRID_SIZE * 2)))
                {
                    DestroyCloud(cloud);
                    continue;
                }
