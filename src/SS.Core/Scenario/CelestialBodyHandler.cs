...
namespace StardustSandbox.Core.Scenario
{
    internal sealed class CelestialBodyHandler(AssetDatabase assetDatabase, GameScreen gameScreen, TimeHandler timeHandler, World world)
    {
        ...
        internal void Update()
        {
            ...
            this.positionX = Convert.ToSingle(gameScreen.ViewportCenter.X - (BackgroundConstants.CELESTIAL_BODY_ARC_RADIUS * Math.Cos(this.angle)));
            this.positionY = Convert.ToSingle(gameScreen.Viewport.Y - (BackgroundConstants.CELESTIAL_BODY_ARC_RADIUS * Math.Sin(this.angle)));
