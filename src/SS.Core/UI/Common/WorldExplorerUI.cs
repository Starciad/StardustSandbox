...
namespace StardustSandbox.Core.UI.Common
{
    internal sealed class WorldExplorerUI : UIBase<WorldExplorerUIDependencies, WorldExplorerUIModel>
    {
        ...
        protected override void OnBuild(UIBuildContext context, WorldExplorerUIModel model)
        {
        ...
        {
            // Background
            this.panelBackground = new(this.assetDatabase.GetTexture(TextureIndex.UIBackgroundWorldExplorer))
            {
        ...
        private void BuildWorldDisplaySlots()
        {
            for (int row = 0; row < UIConstants.WORLD_EXPLORER_ITEMS_PER_ROW; row++)
            {
                ...
                {
                    Image background = new(this.assetDatabase.GetTexture(TextureIndex.UIButtons), new(0, 0, 386, 140))
                    {
