...
namespace StardustSandbox.Core.Databases
{
    ...
    {
        internal int CategoryLength => this.categoryLength;
        internal int ItemLength => this.itemLength;

        private readonly int categoryLength = 0;
        private readonly int itemLength = 0;
        ...
        private readonly Category[] categories = [
            ...
            new(
                ...
                new Subcategory(
                    ...
                    textureIndex: TextureIndex.Icons,
                    sourceRectangle: new(32, 0, 32, 32),
        ...
        internal CatalogDatabase()
        {
            ...
            for (int i = 0; i < this.categories.Length; i++)
            {
                ...
                {
                    Subcategory subcategory = category[j];
                    this.itemLength += subcategory.Length;
                }
            }
        }

        internal Category GetCategory(int index)
        {
            return this.categories[index];
        }

        internal Item GetItem(int categoryIndex, int subcategoryIndex, int itemIndex)
        {
            return this.categories[categoryIndex][subcategoryIndex][itemIndex];
        }

        internal Item[] GetItems(int amount)
        {
            if (amount < 1)
            {
