...
namespace StardustSandbox.Core.Elements.Solids.Immovables
{
    ...
    {
        internal Gold(ElementIndex index, ElementCategory category, ElementRenderingType renderingType, Point textureOriginOffset, Color referenceColor, AchievementManager achievementManager) : base(index, category, renderingType, textureOriginOffset, referenceColor, achievementManager)
        {
        ...
        protected override void OnTemperatureChanged(ElementContext context, float currentValue)
        {
            if (currentValue > 1060.0f)
            {
                ...
                context.SetStoredElementIndex(ElementIndex.Gold);
            }
        }
    }
}
