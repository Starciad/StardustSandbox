...
{
    internal sealed class VolumeMapper : IMapper
    {
        public IData ToData(IStorageModel value)
        {
            VolumeStorageModel storageModel = (VolumeStorageModel)value;

            return new VolumeData()
            {
                MasterVolume = storageModel.MasterVolume,
                MusicVolume = storageModel.MusicVolume,
                SFXVolume = storageModel.SFXVolume,
            };
        }

        public IStorageModel ToStorageModel(IData value)
        {
            VolumeData data = (VolumeData)value;

            return new VolumeStorageModel()
            {
